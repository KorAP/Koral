...
//            classCounter++;
    ...
    private Map<String, Object> parseOPINOptions (Tree node,
        ...
        if (!isExclusion) {
            wrapSecondOperand(classRefCheck, node);
        }
...
//        }
    ...
    private void wrapSecondOperand (ArrayList<ClassRefCheck> classRefCheck,
            Tree node) {
        // if (range.equals("all")) {
...
//         makeResetReference(); // reset all defined classes
        ...
        if (!classRefCheck.isEmpty()
                && !classRefCheck.contains(ClassRefCheck.INCLUDES)) {
            wrapOperandInClass(node, 2, 128 + classCounter++);
        }
        else if (rangenode != null){
